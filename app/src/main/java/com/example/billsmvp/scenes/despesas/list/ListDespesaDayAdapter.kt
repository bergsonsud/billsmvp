...
import kotlinx.android.synthetic.main.fragment_list_despesas_date.view.*
import kotlinx.android.synthetic.main.fragment_list_despesas_row.view.*


class ListDespesaDayAdapter(var transacaoInterface : TransacaoInterface, val context: Context,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val dayView = LayoutInflater.from(context).inflate(R.layout.fragment_list_despesas_date,parent,false)
        val transacaoView = LayoutInflater.from(context).inflate(R.layout.fragment_list_despesas_row,parent,false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
       if (holder.itemViewType==0) {
           holder.itemView.data.text = "Dia "+(lista[position] as TransacaoDate).day.toString()

       }else{
           var transacao = (lista[position] as Transacao)
           holder.itemView.descricao.text = (lista[position] as Transacao).descricao
           holder.itemView.valor.text = (lista[position] as Transacao).valor.formataParaReal()
    ...
    inner class RecyclerViewDateViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView),View.OnClickListener, View.OnLongClickListener{
